...
//
//  1) '유지' 되돌리기 — 백엔드 core.choice_kind 는 이직/창업/진학 3분류이고
//     기본값이 '이직'이다. 프론트가 제공하는 '현상 유지'는 이직으로 계산돼 돌아온다.
//     그래서 유지 슬롯은 기준선 궤적(scenario_trajectories["유지"])으로 바꾸고,
//     이직 전용 산출물(인과효과·생존·유사사례·재이직확률)은 싣지 않는다.
...
function buildSide(scenario, choice, detail, profile) {
  ...
  // '유지'에는 개인단위 이직 모델(L2 매칭·L3 인과·L4 생존)을 적용하지 않는다.
  const hasIndividual = choice !== "유지";
  ...
  return {
    ...
    coverage: hasIndividual
      ? raw.coverage || scenario?.coverage || ""
      : "현상 유지: 생활지표(L1) + 비슷한 사람들의 소득·만족도 궤적(L5) 기준선. 이직 전용 인과·생존 모델은 적용하지 않음",
