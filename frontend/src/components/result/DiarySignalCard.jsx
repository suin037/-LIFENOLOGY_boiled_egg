...
import { computeDiarySignals, valueGap, interpretSignals, domainAnalysis, domainReport } from "../../data/diarySignals.js";
...
export default function DiarySignalCard() {
  const { profile, scenarioDomains } = useResult();
  ...
  const fieldLabel = domainLabel(primaryLife);
  ...
  // 그 분야 일기 분석(그래프·감정·대표 기록)
  const anal = useMemo(() => domainAnalysis(planetKey), [planetKey]);
